...
async def enforce_ban_logic(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int):
    ...
    details = db.get_gban_details(user_id)
    if details:
        try:
            ...
            msg = (f"⚠️ <b>Alert!</b> This user is globally banned.\n"
                   ...
                   f"<b>Reason:</b> {utils.safe_escape(details[0])}\n"
...
async def gban_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    target_id, reason, error = None, None, None
    if update.message.reply_to_message:
        target_id = update.message.reply_to_message.from_user.id
        reason = " ".join(context.args) if context.args else None
    elif context.args:
        target_id, error = await utils.resolve_target(context, context.args[0])
        reason = " ".join(context.args[1:]) if len(context.args) > 1 else None

    if error: await update.message.reply_text(error); return
    if not target_id or not reason: await update.message.reply_text("Usage: /gban <ID/@user/reply> <reason>"); return
    ...
    old_details = db.get_gban_details(target_id)
    ...
    if db.add_to_gban(target_id, admin.id, reason):
        ...
        hashtag = "#GBANUPDATE" if old_details else "#GBANNED"
        ...
        if old_details: log_message += f"<b>Old Reason:</b> <code>{utils.safe_escape(old_details[0])}</code>\n"
        ...
        if LOG_CHANNEL_ID: await context.bot.send_message(LOG_CHANNEL_ID, log_message, parse_mode=ParseMode.HTML)

async def ungban_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    admin = update.effective_user
    chat = update.effective_chat
    if not db.is_sudo(admin.id): return
    target_id, error = None, None
    if update.message.reply_to_message: target_id = update.message.reply_to_message.from_user.id
    elif context.args: target_id, error = await utils.resolve_target(context, context.args[0])
    
    if error or not target_id: await update.message.reply_text("Skrrrt... I can't find the user."); return
    ...
    if db.remove_from_gban(target_id):
        ...
        await update.message.reply_html(log_message)
        if LOG_CHANNEL_ID: await context.bot.send_message(LOG_CHANNEL_ID, log_message, parse_mode=ParseMode.HTML)
    else: await update.message.reply_text("User is not globally banned.")

async def gban_stat_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    is_admin = db.is_sudo(user.id)
    target_id, checking_self = None, False

    if is_admin:
        if update.message.reply_to_message: target_id = update.message.reply_to_message.from_user.id
        elif context.args: target_id, _ = await utils.resolve_target(context, context.args[0])
...
async def add_sudo_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        t_id, err = await utils.resolve_target(context, context.args[0]) if context.args else (None, None)
        if update.message.reply_to_message: t_id = update.message.reply_to_message.from_user.id
        if t_id:
            db.add_sudo(t_id)
            await update.message.reply_text(f"✅ Added {t_id} to sudo.")
    except: await update.message.reply_text("Usage: /addsudo <ID/@user/reply>")
...
async def del_sudo_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        t_id, err = await utils.resolve_target(context, context.args[0]) if context.args else (None, None)
        if update.message.reply_to_message: t_id = update.message.reply_to_message.from_user.id
        if t_id and t_id != OWNER_ID:
            db.remove_sudo(t_id)
            await update.message.reply_text(f"❌ Removed {t_id} from sudo.")
    except: await update.message.reply_text("Usage: /delsudo <ID/@user/reply>")
